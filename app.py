...
def get_gspread_client():
    try:
        # 1. تحديد الصلاحيات المطلوبة للوصول لملفات جوجل
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        
        # 2. البحث عن "الصندوق" الذي يحتوي على البيانات في إعداداتك
        if "gcp_service_account" in st.secrets:
            # 3. استخراج نص الـ JSON من داخل gcp_service_account وتحويله لكود مفهوم
            json_string = st.secrets["gcp_service_account"]["json_data"].strip()
            service_account_info = json.loads(json_string, strict=False)
            
            # 4. بناء الهوية والاتصال
            creds = Credentials.from_service_account_info(service_account_info, scopes=scope)
            return gspread.authorize(creds)
        else:
            st.error("⚠️ لم يتم العثور على gcp_service_account في الإعدادات السرية!")
            return None
    except Exception as e:
        st.error(f"❌ حدث خطأ في محاولة الاتصال: {e}")
        return None
...
if not st.session_state.logged_in:
  ...
  if inventory is not None and not inventory.empty:
      for item, qty in inventory.items():
          if qty > 0:
              ...
              st.markdown(f'<div class="stock-card"><div style="display:flex; justify-content:space-between; align-items:center;"><span style="font-size:20px; font-weight:800;">{item}</span><span style="font-size:24px; color:{qty_color}; font-weight:900;">{int(qty)}</span></div></div>', unsafe_allow_html=True)
  else: st.info("لا توجد بضاعة حالياً.")
  ...
  if st.session_state.receipt_view:
      ...
      if sel_p != "-- اختر --":
          ...
          st.info(f"💡 الرصيد المتوفر في سيارتك: {int(stock_val)}")
